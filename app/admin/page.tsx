...
const ADMIN_PASSWORD = 'nbagoat2026'
  ...
  function FileCard({label,file,inputRef,onChange}:{label:string;file:File|null;inputRef:React.RefObject<HTMLInputElement|null>;onChange:(f:File|null)=>void}) {
    return (
      <div className="card" style={{padding:20,display:'flex',flexDirection:'column',gap:12}}>
        <div style={{fontFamily:'var(--font-head)',fontSize:16,letterSpacing:'0.05em',color:'var(--text)'}}>{label}</div>
        <div style={{fontSize:12,color:'var(--text2)',lineHeight:1.6}}>
          Same column layout as the initial Stathead export. New rows upsert — existing (player, date) pairs are replaced.
        </div>
        <div
          style={{border:'2px dashed var(--border2)',borderRadius:8,padding:'28px 20px',textAlign:'center',cursor:'pointer',background:file?'var(--blue-dim)':'transparent',transition:'background .2s'}}
          onClick={()=>inputRef.current?.click()}
        >
          {file ? (
            <div><div style={{color:'var(--blue)',fontWeight:600,marginBottom:4}}>✓ {file.name}</div><div style={{fontSize:12,color:'var(--text2)'}}>{(file.size/1024).toFixed(1)} KB</div></div>
          ) : (
  ...
  return (
    <div style={{maxWidth:720,margin:'40px auto',padding:'0 24px'}}>
      <div style={{marginBottom:28}}>
        <h1 style={{fontSize:32,color:'var(--blue)',marginBottom:6}}>BATCH UPLOAD</h1>
        <p style={{color:'var(--text2)',fontSize:14,lineHeight:1.6}}>
          Upload new game CSVs as the playoffs progress. The processor derives series context, computes era-adjusted GmSc, and upserts all rows. Always upload both files together so series context stays accurate.
        </p>
      </div>
      ...
      <button className="btn btn-blue" style={{padding:'10px 24px',fontSize:14,justifyContent:'center',width:'100%'}}
        disabled={uploading||(!playerFile&&!teamFile)} onClick={handleUpload}>
        {uploading ? <><div className="spinner" style={{width:14,height:14,borderTopColor:'#fff'}}/>Processing…</> : 'Process & Upload'}
      </button>

      {result && (
        <div style={{marginTop:20,padding:16,borderRadius:8,background:result.type==='success'?'var(--green-dim)':'var(--red-dim)',border:`1px solid ${result.type==='success'?'rgba(15,123,78,.2)':'rgba(197,48,48,.2)'}`}}>
      ...
      <div style={{marginTop:32,fontSize:13,color:'var(--text2)',lineHeight:1.8}}>
        ...
        <div style={{fontFamily:'monospace',fontSize:11,background:'var(--surface2)',padding:16,borderRadius:8,lineHeight:2,border:'1px solid var(--border)'}}>
          <div style={{color:'var(--blue)',fontWeight:600}}>Player Games (column order matters):</div>
          ...
          <div style={{marginTop:8,color:'var(--blue)',fontWeight:600}}>Team Games (column order matters):</div>
...
export default function AdminPage() {
  ...
  if (!authed) {
    return (
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'calc(100vh - 52px)',background:'var(--bg)'}}>
        <div className="card" style={{padding:40,width:360,textAlign:'center'}}>
          <div style={{fontFamily:'var(--font-head)',fontSize:28,color:'var(--blue)',marginBottom:6}}>ADMIN</div>
          <div style={{fontSize:13,color:'var(--text2)',marginBottom:24}}>Enter password to access upload tools</div>
          <input
            type="password"
            ...
            autoFocus
          />
          {error && <div style={{fontSize:12,color:'var(--red)',marginBottom:10}}>Incorrect password</div>}
          <button className="btn btn-blue" style={{width:'100%',justifyContent:'center'}} onClick={tryLogin}>
            Enter
          </button>
        </div>
      </div>
